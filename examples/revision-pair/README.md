...
Two revisions of one specification, for `ess verify diff` and `ess verify impact`.
...
```console
ess verify conform synthesize --path examples/revision-pair/before --out target/catalog-suite.json
ess verify impact --from examples/revision-pair/before --to examples/revision-pair/after \
  --suite target/catalog-suite.json
```
