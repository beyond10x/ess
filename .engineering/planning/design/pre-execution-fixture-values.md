...
id: design:pre-execution-fixture-values
kind: design
status: draft
...
The candidate allocates source `ess/7`, authored `ess-scenario/3`, ordinary suite `ess-conformance/12` and coverage suite `ess-conformance/13`. These are unreleased additions. Browser replay refuses independently provisioned fixtures before emitting an artifact.
