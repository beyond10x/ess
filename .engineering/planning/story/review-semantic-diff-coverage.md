...
Implementation confirmation at 25580795fa0fef04563a94f9864745377b871ee1 and correction 5c009bfc2f2d306b445d86505099eb88af1df23b (source: docs/reviews/2026-09-05-review-boundaries-4-semantic-implementation.md; semantic-correction-1.md; review-result:review-boundaries-4-semantic-adversary-pass-1):

- cited — the four reserved packages were sufficient. Compiler production changes are in the graph owner; ess-diff owns comparisons, typed changes, delta readers/writers and impact policy; ess-gen owns sliced provenance and its new private stamp.rs reader. ess-synth needed only HTTP and actual Cargo emission integration tests. The inferred synthesis package scope is confirmed; no synthesis production edit was required.
- cited — the design addendum now binds diff/2, impact/3 and slice-sha256/2 for Constructs. The earlier Scope current-contract references describe the scoped base, not these new defaults. Exact diff/1 bytes/vocabulary remain readable/writable with explicit version admission; new changes cannot be relabeled legacy. No impact reader was introduced.
- cited — all eight new typed change subtypes and independent residual accounting are implemented. The first review proved that pruning erased residual owner identity; the correction retains the name of every named-list member with surviving unexplained content. Four independent counterexamples and classified-only/effective-name/parsed-predicate controls pass unchanged.
- cited — graph vocabulary grows from 21 to 25 relations with relation-target, ownership-carrier, exposes-view and parameter-type. Forward slices and reverse impact over both revisions retain their direction. Ownership carrier cycles terminate through the existing visited set. Source and whole-model hashing remain frozen.
- cited — stamp admission requires complete writer envelopes, exact supported profiles and unambiguous authoritative positions. The first review showed incomplete schema attribution accepted by the actual impact caller; it now yields ProvenanceUnreadable. A separate correction probe measured 14 actual Rust/Web Cargo manifest headers rejected before framing repair and readable afterward. This does not validate TOML or claim an individual-manifest CLI impact caller. Clap framing is source-backed only.
- cited — every reserved generated path has measured changes: 37 Billing projections (2 AsyncAPI,2 docs,2 OpenAPI,29 schemas,2 site files),5 package corpus domain pages, and4 Gatepass HTTP payloads. The latter OpenAPI payloads also correct existing x-ess-entities/whitespace drift: the actual old emitter already produced the table. Each original and regenerated file hash is retained. Both HTTP neutral plans and all other emitted HTTP files match. No generated byte changed during correction.
- cited — the full 154846-byte pre-F01 Billing suite/4 fixture (30 scenarios,one authored,zero refusals) is committed under ess-diff tests and the candidate writer/reader roundtrip matches exactly. Whole index/source/contract controls remain frozen. This is unchanged-input compatibility, not complete conformance coverage or a future-format claim.
- cited — no root Cargo manifest/lock, CLI/xtask, conformance production, composition production, public guide or website source edit was needed. generated/site is outside the public website source allowlist.
- cited — Atlas ADR0036 is published at 974b2a2bc4896bd76293a734f36ac254895221c4. Its consumer catalog-intent obligation stays open; ESS-only observations refreshed, while actual SDK/AgentIDE source pins are independently inventoried. The SDK old baseline and actual generated compilation passed; candidate comparison is still pending the final reviewed frozen source. Neither a published SDK pin change nor deployment is claimed.
