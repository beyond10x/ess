...
publication into the checkout can never pass there. Observed 2026-09-10, babelconnect-app pipeline
...
```
$ scripts/ess-pinned.sh generate --path babelconnect-specs --kind site --out public --include …
...
- Adopter record: babelconnect-app `.gitlab-ci.yml` `pages` job comment and the `getfattr -d -m - .`
