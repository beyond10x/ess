...
| 2 | `continues_a_path` accepts ASCII only, so a home path whose account name **starts** with a non-ASCII byte is collected as nothing: `/home/ärni/.cache/ess/report.json` → nothing. Only the first byte decides — `…/jösé/…` is still caught, truncated. | constructed; no such tracked path |
