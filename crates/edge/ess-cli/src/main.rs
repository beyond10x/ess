...
enum SpecifyCommand {
    ...
    Compose {
        ...
        /// One enclosing ownership anchor for every composition output.
        #[arg(long)]
        ownership_root: Option<PathBuf>,
        /// Output rendering.
...
enum GenerateCommand {
    /// Explicitly adopt generated bytes or recover an interrupted publication.
    Output {
        #[command(subcommand)]
        command: output_ownership::Command,
    },
    ...
    match command {
        ...
        RealizationCommand::Generate { .. } => {
            ...
            if generate == Some(true) {
                let _guard = output_ownership::check(
                    path.parent()
                        .filter(|p| !p.as_os_str().is_empty())
                        .unwrap_or(Path::new(".")),
                )?;
                preflight_named_output(path)?;
                ...
                output_ownership::named(path, "realization-markdown", &markdown)?;
...
fn build(command: BuildCommand) -> Result<ExitCode> {
    match command {
        ...
        BuildCommand::Execute {
            ...
            write_owned_projection_files(&projection_out, "buildkit", projection.files())?;
...
fn compose(
    ...
    publish_composition_outputs(outputs, ownership_root)?;
...
fn publish_composition_outputs(
    outputs: Vec<(PathBuf, &str)>,
    ownership_root: Option<&Path>,
) -> Result<()> {
    // Both named companions and the client tree share one complete collision/ownership set.
    preflight_output_set(outputs.iter().map(|(path, _)| path.as_path()))?;
    if outputs.is_empty() {
        return Ok(());
    }
    let anchor = resolve_output_directory(
        ownership_root.context("writing composition output requires --ownership-root")?,
    )?;
    let relative = outputs
        .into_iter()
        .map(|(path, contents)| {
            Ok((
                path.strip_prefix(&anchor)
                    .with_context(|| {
                        format!(
                            "composition output {} is outside --ownership-root {}",
                            path.display(),
                            anchor.display()
                        )
                    })?
    ...
    let artifacts = match kind {
        ...
        None => {
            ...
            for generator in ess_gen::generators() {
                let files = ess_gen::artifact::run(generator.as_ref(), &ir)?;
                publications.push(output_ownership::Publication::tree(
                    &format!("projection:{}", generator.name()),
                    files
                        .values()
                        .map(|a| (a.path.as_str(), a.contents.as_str())),
                )?);
                for (path, artifact) in files {
                    if all.insert(path.clone(), artifact).is_some() {
                        bail!("duplicate generated projection path: {path}");
                    }
                }
            }
            all
...
fn synthesize(
    ...
    write_owned_artifacts(out, "synthesis", &synthesis.artifacts)?;
    if matches!(format, Format::Text) {
...
fn project(adapter: ProjectAdapter) -> Result<ExitCode> {
    match adapter {
        ProjectAdapter::Buildkit { ir, out } => {
            ...
            write_owned_projection_files(&out, "buildkit", projection.files())?;
            println!(
        ...
        ProjectAdapter::Helm {
            ...
            write_owned_projection_files(&out, "helm", projection.files())?;
            println!(
...
fn write_owned_files<'a>(
    ...
    output_ownership::publish(
        root,
        vec![output_ownership::Publication::tree(family, files)?],
    )
}
fn write_owned_artifacts(
    out: Option<&Path>,
    family: &str,
    artifacts: &std::collections::BTreeMap<String, ess_gen::Artifact>,
) -> Result<()> {
    let publication = output_ownership::Publication::tree(
        family,
        artifacts
            .values()
            .map(|a| (a.path.as_str(), a.contents.as_str())),
    ...
    if let Some(root) = out {
        output_ownership::publish(root, vec![publication])?;
    }
    Ok(())
}
fn write_owned_projection_files(
    root: &Path,
    family: &str,
...
fn project_openapi_interface(
    ...
    if let Some(path) = out {
        output_ownership::named(path, "openapi-file", &yaml)?;
    }
    match (out, format) {
...
fn project_kubernetes(
    ...
    if let Some(root) = out {
        write_owned_projection_files(root, "kubernetes", &artifacts)?;
    }
    match format {
