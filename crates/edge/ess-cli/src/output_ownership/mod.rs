...
fn initialize(
    ...
    let payload = Payload {
        ...
        producer: Some(state::PRODUCER.to_owned()),
    };
...
fn publish_observers(
    ...
    // Only a publication that changes the tree reaches here, so the note attributes a real
    // difference; the first checkpoint below records this release (ess#106).
    if let Some(previous) = payload.produced_here() {
        eprintln!(
            "note: {} was last generated by {previous}; this is {}, so a difference in it may \
