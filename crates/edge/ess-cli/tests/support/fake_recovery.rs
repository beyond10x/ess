...
//!
//! Its [`fake_helm`] module is the **binary**: the stand-in for the admitted Helm artifact *and*
//! the synthetic target, built as `ess-recovery-fake` from that file alone so that it links none
//! of the product. The target's state is a directory of its own: it outlives the driver process
//! that mutated it, it is never reconstructed from the executor's journal, and that independence
//! is what makes "the target changed and the run did not record it" a real state rather than an
//! assertion.
//!
...
//
// Both live in `fake_helm.rs`, which is also the `ess-recovery-fake` executable. It links none of
// the product, because every engine-level case hashes and probes a copy of it; the fixture reaches
// its target reader, writer and profile through this module, so the two still share one copy.

#[path = "fake_helm.rs"]
mod fake_helm;

// Each file that includes this one uses a different part of it, as with `dead_code` above.
#[allow(unused_imports)]
pub use fake_helm::{read_release, write_profile, write_release, Profile, ReleaseState, PROFILE};
