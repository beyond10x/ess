...
pub fn continues_a_path(character: char) -> bool {
    character.is_alphanumeric() || matches!(character, '-' | '_' | '.' | '/' | '+' | '@')
}

/// The lane's `begins_an_absolute_path`, copied.
pub fn begins_an_absolute_path(text: &str, start: usize) -> bool {
...
/// The lane's `without_comment`, copied.
pub fn without_comment(value: &str) -> &str {
    match value.find(" #") {
...
/// The lane's `names_a_runner`, copied.
pub fn names_a_runner(label: &str) -> bool {
    !label.is_empty()
        && !label.contains(char::is_whitespace)
        && !label.contains(['$', '{', '}', '#', '[', ']', ',', '"', '\'', ':'])
}

/// The lane's `indentation`, copied.
pub fn indentation(line: &str) -> usize {
    line.len() - line.trim_start().len()
}

/// The lane's `block_sequence`, copied.
pub fn block_sequence(lines: &[&str], from: usize) -> Vec<String> {
...
/// The lane's `mapping_value`, copied.
pub fn mapping_value(lines: &[&str], from: usize, key: &str) -> Vec<String> {
...
/// The lane's `enclosing_block`, copied.
pub fn enclosing_block(lines: &[&str], number: usize) -> (usize, usize) {
...
/// The lane's `matrix_values`, copied.
pub fn matrix_values(lines: &[&str], key: &str) -> Vec<String> {
...
/// The lane's `runners_per_line`, copied.
pub fn runners_per_line(workflow: &str) -> Vec<(usize, BTreeSet<String>)> {
...
/// The lane's `runner_labels`, copied.
pub fn runner_labels(workflow: &str) -> BTreeSet<String> {
...
/// The lane's `ci_runner_labels`, copied.
