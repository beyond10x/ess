...
fn render(root: &Path) -> Result<String> {
    ...
    row(&mut output, "Explicit executors", "`execute`, `publish`, `fetch`, `reconcile` invoke external clients; reconciliation compares an admitted baseline desired deployment with the desired one, and `--authority` selects a protected registry entry for the finite recovery contract", &format!("Caller-supplied state and credentials remain material; a supplied baseline is admitted intent rather than proof of application, and neither it nor a completed call is a continuous control plane or an automatic recovery proof. {}. The support check invokes none of these verbs.", source("CLI executor owner", "crates/edge/ess-cli/src/main.rs")));
