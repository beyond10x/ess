...
fn render(root: &Path) -> Result<String> {
    ...
    row(&mut output, "OpenAPI adapter", "Supported 3.0 and 3.1 service/interface import to `ess-openapi-import/1`, retaining source and accounting; checked projection", &format!("External references refuse. Semantic gaps, unresolved references and legacy interface-only inputs block checked projection; annotation normalization alone may be allowed. {} and {}.", source("accounting tests", "crates/generate/ess-openapi/tests/accounting.rs"), source("import/refusal owner", "crates/generate/ess-openapi/src/lib.rs")));
