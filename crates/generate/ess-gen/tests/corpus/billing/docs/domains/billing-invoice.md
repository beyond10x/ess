...
contract digest slice-sha256/2:a21fd36f0055057629f4c235962163cdd34a3d178aa068925bcb53be623af301
...
Every instance satisfies `total.amount >= 0` and `reminder_count >= 0` — a predicate over this entity's own fields, checked against them rather than stored as a sentence, so an invariant reading something the entity does not have is refused instead of documented.
...
Its rows are ordered by `issued_at` descending.
...
**`accepted`** — The invoice is created in Draft. Taken when `amount.amount > 0` holds of the input. It creates a `billing.invoice.Invoice`, which starts in `Draft`. The new instance's identity is published as `invoice_id` on `billing.invoice.InvoiceCreated`. It emits `billing.invoice.InvoiceCreated`. It sets `account_id` from `input.account_id`, `total` from `input.amount` and `reminder_count` from `"0"`. A test reaches it by constructing an input that satisfies that condition.
...
Generated from billing v3 · model digest `1e7906786567af32118eb2d0a8c3fcafa16c32c9649a80b60487fd2eeebc4c9c` · contract digest `slice-sha256/2:a21fd36f0055057629f4c235962163cdd34a3d178aa068925bcb53be623af301`. Do not edit this file; change the specification and regenerate it with `ess generate`.
