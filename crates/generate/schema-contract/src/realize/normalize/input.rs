...
use super::{NumberPath, PositionalInput, Refused};
...
pub(super) fn parse(
    ...
    decode(
        ...
        &captures.iter().map(Vec::as_slice).collect::<Vec<_>>(),
        &positions
            .iter()
            .map(|policy| (policy.path.as_slice(), policy.length))
            .collect::<Vec<_>>(),
    )
}
...
fn decode(
    ...
    paths: &[&[NumberPath]],
    captures: &[&[NumberPath]],
    positions: &[(&[NumberPath], u64)],
) -> Result<Value> {
    if captures.iter().any(|path| path.is_empty()) {
        return super::retained::capture(raw, at, depth).map(Value::String);
    }
    if depth > 64 {
        return Err(error(at, "input_depth", "JSON input exceeds 64 levels"));
    }
    if let Some((_, length)) = positions.iter().find(|(path, _)| path.is_empty()) {
        return positional(raw, at, depth, *length);
    }
    let text = raw.get();
            ...
            object
                ...
                .map(|(key, value)| {
                    decode(
                        ...
                        &path(at, key),
                        depth + 1,
                        &descend(paths, Some(key)),
                        &descend(captures, Some(key)),
                        &descend_positions(positions, Some(key)),
                    )
            ...
            values
                ...
                .map(|(index, value)| {
                    decode(
                        ...
                        depth + 1,
                        &descend(paths, None),
                        &descend(captures, None),
                        &descend_positions(positions, None),
                    )
                })
                .collect::<Result<_>>()
                .map(Value::Array)
...
pub(super) fn require_positional_text(policies: &[PositionalInput]) -> Result<()> {
...
fn descend_positions<'a>(
    positions: &[(&'a [NumberPath], u64)],
    field: Option<&str>,
) -> Vec<(&'a [NumberPath], u64)> {
    positions
        .iter()
        .filter_map(|(path, length)| match (path.split_first(), field) {
            (Some((NumberPath::Field { name }, rest)), Some(field)) if name == field => {
                Some((rest, *length))
            }
            (Some((NumberPath::Items, rest)), None) => Some((rest, *length)),
            _ => None,
        })
        .collect()
}

fn positional(raw: &RawValue, at: &str, depth: usize, length: u64) -> Result<Value> {
    let kind = raw.get().as_bytes()[0];
    if kind != b'[' && kind != b'n' {
        return Err(error(
            at,
    ...
    if kind == b'[' {
        ...
        for (index, value) in values.iter().enumerate() {
            ...
            let value = match value.get().as_bytes()[0] {
                b'n' => String::new(),
                b'"' => serde_json::from_str::<String>(value.get()).map_err(|_| {
                    error(
                        &at,
                        "input_syntax",
    ...
    // The sealed policy already equated length to an allocated schema prefix.
    while (result.len() as u64) < length {
        result.push(Value::String(String::new()));
    }
