...
//! `crates/specify/ess-compiler/tests/billing.rs` reads for them.
//!
...
fn no_source_file_in_the_diff_engine_calls_an_ir_handle_accessor() {
    ...
    //
    // Every accessor `crates/specify/ess-compiler/src/ir.rs` generates, spelt as the call would be. The list
...
fn a_delta_written_in_a_format_this_build_does_not_read_is_refused() {
    let mut document = document();
    document["format"] = serde_json::json!("ess-diff/99");
