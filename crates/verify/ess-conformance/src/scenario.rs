...
//! (`crates/specify/ess-compiler/src/ir.rs`). Inside one process that is a programming mistake, not a
...
pub struct SuiteProvenance {
    ...
    ///
    /// Wave 7 (W7.1). Every generated artifact now carries beside its model digest the digest of
    /// the slice it derives from, and for a suite the slice is legitimately the whole model: a
    /// scenario exists for every command outcome, every lifecycle move and every view the
    /// specification declares, so there is no construct whose change provably cannot oblige a
...
// The semantic reference vocabulary lived here from wave 4, when a committed suite was the first
// document that had to name a construct outside the process that compiled it. Wave 7 moved it into
// `ess-compiler` beside the IR the names are minted from, because the dependency graph and the
// generated artifacts' slice digests use the same vocabulary now — and a suite's spelling of a
// name must be the graph's spelling, or "which scenarios depend on `billing.invoice.Money`" and
// "which artifacts derive from it" become two questions in two languages. Re-exported here so a
// suite's readers and writers did not move.
pub use ess_compiler::refs::{
