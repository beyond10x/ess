...
//
// One paired input, emitted beside it:
//
//   replay.json  the closed model projection, exact suite reference and complete input/1 lineage
//
// Every transition the player applies is read from the admitted model — an outcome names its transition
...
const originalReplay = await fetch('replay.json').then(async (response) => {
  if (!response.ok) throw new Error('replay input unavailable')
  return new TextDecoder('utf-8', { fatal: true, ignoreBOM: true }).decode(await response.arrayBuffer())
})
const admitted = await admitReplay(originalReplay)
const { model, suite } = admitted
const banner = document.createElement('section')
banner.id = 'coverage'
banner.textContent = admitted.description
