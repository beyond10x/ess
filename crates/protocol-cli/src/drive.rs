...
fn start(args: &RunArgs) -> Result<ExitCode> {
    ...
    if let Some(refusal) = machine_preflight(&inputs.map) {
        outln!("{refusal}");
        return Ok(ExitCode::from(1));
    }
...
/// The pre-flights that say *this machine cannot run this map today*, in the order they are asked.
///
/// They sit behind the coverage check, never in front of it, and that order is load-bearing: a
/// coverage gap is true on **every** machine and a missing binary is true only on this one. With
/// the machine checks first, a map with a real coverage gap read as fine wherever a binary was
/// missing.
///
/// Three questions and two `PATH`s. The first two are about the **session's** `PATH` — the one
/// metaharness constructs for an `llm` step — and the third is about the **driver's** own, which is
/// the operator's shell. That they are different is why `W4-3/1`'s `command` step ran a binary four
/// releases stale while a guard that looked like it covered this was passing.
fn machine_preflight(map: &StepMap) -> Option<String> {
    metaharness_preflight(map)
        .or_else(|| {
            has_llm_steps(map)
                .then(protocol_on_the_session_path)
                .flatten()
        })
        .or_else(|| protocol_command_preflight(map))
}
