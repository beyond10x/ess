...
| `ess generate schema normalize-generate --recipe FILE --bundle FILE… --target rust\|go --package NAME [--module PATH] --out DIR [--check]` | Emit a source-pinned normalization library or check planned file bytes without writing. Go requires `--module`; Rust refuses it. TypeScript is not substituted by structural declarations. Unreleased. |
