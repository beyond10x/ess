...
- Return checked `Result` values from Rust/Web workspace emission and both synthesis facades.
  Unsupported Rust representations produce source-addressed `ess-target-failure/1` errors before
  artifacts or CLI output writes. Valid generated bytes and neutral plans remain unchanged.
- Make generated Web workspaces compile for systems with no deliveries, no published events,
  and for catalog-only models; preserve the existing browser catalog contract.
- Repair generated HTTP/Web map decoders for Integer, Boolean and Bytes keys, including nested
  optional and collection types.
- Document current format versions, reader boundaries, canonical bytes and digest identities,
  and update the change-tracking guide for the current diff and impact commands.
