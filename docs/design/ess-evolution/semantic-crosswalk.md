...
| `InputFacts` projects collections, newtypes and named fields; view `row_facts` has a smaller projection | ER references walk nested object properties; schema validation checks references at registration | Preserve the projection appropriate to the ESS construct. `runner.rs::bind` omits sequences and nulls; this existing view observer limit is not evidence that input quantification is unsupported. Map keys/list ordinals/quantifier scopes need explicit typed access. |
