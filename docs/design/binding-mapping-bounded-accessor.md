...
### Complete native-target failure vocabulary

An accessor output-budget refusal adds the closed `accessor-resource` cause. Do not
serialize it under the existing `ess-target-failure/1` or `/2` vocabularies. Select
`ess-target-failure/3` for every target when the originating IR contains an
`EventAccessor`, or when any cause is `accessor-resource`. This also covers an early
failure with an existing cause, such as an invalid target identifier, on an accessor
model. Pass the originating IR to the private failure constructor so an early return
cannot lose the feature boundary.

Without an accessor or new cause, preserve the exact existing `/1` Rust/Web and `/2`
Go/Clap output bytes and cause precedence. The neutral `SynthesisPlan` does not itself
serialize the accessor DAG; the conditional boundary accounts for the originating
feature and the new closed failure cause, without inventing such a plan field. The
failure retains its existing envelope fields and publishes no generated artifacts.
Targeted serializer and CLI witnesses must cover both the new cause and an old-cause
early failure on accessor input, alongside unchanged legacy failures. This is an
implementation-time format consequence of the already required bounded native
refusal, not a new accessor capability or an extra full design-review round.
