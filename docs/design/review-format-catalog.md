...
| Suite: **`provenance.suite_version: ess-conformance/4`** | Specification `vN`, `spec_digest`, whole-model `contract_digest`; no exact-suite digest | Synthesizers/authored compiler → `ConformanceSuite`; ordinary derived Deserialize/from_json. `SuiteFormat` parses syntax; declared support `[1,2,3,4]` is not a universal execution-admission boundary. Rust runner and reduced Go DTO do not independently enforce all that support policy. [Suite](../../crates/verify/ess-conformance/src/scenario.rs), [runner](../../crates/verify/ess-conformance/src/runner.rs), [Go](../../crates/verify/ess-conformance/src/go/runtime.go). | P; both provenance digests remain bare hashes. No current digest of the complete suite bytes, durable complete-coverage claim or uniform closed outer DTO. |
...
| Detailed run: **unversioned** `ConformanceReport` | Suite provenance, implementation, run/scenario identities and timestamps | Rust runner → detailed CLI JSON/YAML; distinct from the standalone JSON written by `--report-out`. Serialize-only detailed result; no current `ess-conformance-run` discriminator. [Report](../../crates/verify/ess-conformance/src/report.rs), [CLI](../../crates/edge/ess-cli/src/main.rs). | P canonical method / CLI JSON, Y presentation; referenced suite/model identities, no exact-suite/report digest. |
...
| Intent: `format: infra-spec/1` | Human-readable `name`, not release/version identity | `infra_spec::read_spec`: JSON/YAML → raw closed shapes → `TryFrom` checks marker, scope, predicates and remedies. Typed `InfraSpec` is Serialize-only; no CLI canonical source writer. [Reader](../../crates/infra/infra-spec/src/raw.rs), [type](../../crates/infra/infra-spec/src/spec.rs). | Authored bytes: —; no intent digest API. |
...
| Projection JSON: `format: infra-projection/1`, **`artifacts`** | Intent name and projection provenance | `project` returns checked `Result`; `Projection::document`/`to_json` emits `ProjectionDocument`, including emitted artifact contents; no reader. [Owner](../../crates/infra/infra-project/src/project.rs). | K; referenced inputs, no projection digest. |
| Projection YAML / direct typed serialization: `format: infra-projection/1`, **`patches`/`objects`** | Same intent/provenance | CLI YAML serializes `Projection` directly, not `ProjectionDocument`; no reader. The marker is shared with the distinct JSON output shape. [Type](../../crates/infra/infra-project/src/project.rs), [CLI](../../crates/edge/ess-cli/src/main.rs). | Y in CLI; no additional digest. Artifact JSON files use the separate patch writer. |
...
| Exact-suite bytes, **planned only** | `SuiteReference.digest_profile: sha256-json-bytes/1` in the successor conformance design. [Design](review-conformance-coverage.md). | No current suite/4/report/1 writer or reader implements this contract. |
...
Current writers remain suite **/4**, standalone report **/1**, and unversioned detailed runs.
The [binding conformance design](review-conformance-coverage.md) reserves suite **/5**, standalone
report **/2**, detailed **ess-conformance-run/2**, durable coverage/selection and exact-suite binding.
Its 75 matrix rows and 10 version/mode pairings are implementation obligations. Reader/opt-in phases
retain current defaults; counts-only v2 reports over legacy suites remain diagnostic with unknown
coverage. The v2 timestamp contract is exact unsigned decimal u64 with checked adapters; current
Rust u64 and Go int64 timestamps are retained as separate legacy domains.
