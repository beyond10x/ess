...
capture, invariant execution or TypeScript normalization. The separately versioned
[format-4 lexical capture design](raw-json-normalization.md) now binds and implements
explicit raw token capture and retained-document decoding in reference, Rust and Go.
It adds no expression operation and preserves legacy generated bytes through five
frozen templates; positional arrays, invariant execution and TypeScript remain outside
that unit.
