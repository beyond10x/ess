...
## Authored state-name collision correction, 2026-09-16

The same class as the snapshot correction below, one declaration further along, and found the same
way: by a real specification refusing to emit.

An entity's lifecycle enum is `X.State`, filed in `ir.types()` like an authored type. A target with
no dotted type names flattens both it and an authored `XState` to one identifier, and the
feasibility inventory refused the workspace. A view named for the entity whose state it projects is
an ordinary thing to write — `acd.routing.CallState` over `acd.routing.Call` — so this refused
models that are legal, compile, and pass `ess specify validate`. `--target go` emitted the same
workspace, which is how the model could be legal and unemittable at once.

**The snapshot correction's remedy does not transfer.** There, the emitter could choose a second
spelling for a name it derived, because `XSnapshot` is the emitter's to move. Here both candidates
are meaningful: `X.State` is the lifecycle a target must name, and `XState` is what an author
called their view. An emitter picking between them picks between two deliberate names with no
basis, and the model's own rule — a declared name comes from the specification — says it may not.

So the author picks, with `naming: { code: … }`. The refusal stays and names the remedy. Three
consequences worth recording:

- **It is not a per-target map.** A name that differs between Rust and Go is two names to keep in
  step, and the collision is a property of flattening, which every such target does identically.
- **One reader**, `ess_synth::alias::code_aliases`, feeding both `rust::Layout::type_name` and
  `go::Layout::declare`, so the two emitters cannot disagree about a name somebody wrote down.
- **`go` was repairing this backwards.** `allocate_declared` swept `ir.types()` first, so the
  derived enum took the identifier and the author's declaration was renamed `XState_` — the exact
  inverse of the rule `allocate_names` states in its own doc comment, with no test covering it.
  Authored names are now reserved before derived ones.

`entity_snapshots` also gained `XState` in its reserved set. It reserved `XData`, `AnyX` and
`x_state` and not the enum, so two repairs could have landed on one identifier; no committed
specification reaches it.

No generated bytes change for a specification that does not collide, which is the same property the
snapshot correction claimed and got.

## Entity snapshot allocation correction, 2026-09-08

Atlas story:generated-service-persistence-proof owns this narrow correction. AgentIDE declares
ContextPin, ApprovalCheckpoint and Workbench entities alongside authored views whose names end
in Snapshot. The unchanged producer refuses all three because its derived entity boundary structs
take the authored view names. Two regression cases reproduce that refusal; the uncontested-name
control passes before the correction.

Allocate entity snapshot names once in Layout, per owning domain, and use that same allocation in
feasibility checks and rendering. Preserve `EntityNameSnapshot` whenever it is available. When an
authored declaration or another fixed entity helper occupies it, try `EntityNameEntitySnapshot`,
then suffixes `2`, `3` and onward until free. Reserve all authored declarations, fixed helpers and
every entity's ordinary snapshot candidate before choosing fallbacks in qualified-name order, so
a fallback cannot take another entity's existing snapshot name. Independent domains allocate
independently. Authored declarations and all other generated helpers retain their names and
their existing collision checks.

This admits previously refused Rust inputs without changing successful generated bytes, the
neutral synthesis plan, qualified view identities, wire names or persisted projection identities.
No ESS format migration is required: no existing successful representation or contract changes.
Verification must cover the three authored view names, occupied fallback names, preservation of
another entity's ordinary snapshot, source-order determinism, fresh generated-workspace
compilation and the existing projection and refusal gates.
...
### Integration preview: source wire identity

The source-driven integration adds the separate
[unique wire field invariant](unique-wire-field-identity.md). It rejects duplicate effective
JSON object keys during specification validation because schema projection would otherwise
silently overwrite a declared property. This is a source-contract defect independent of Rust
compilability, not a new Rust target restriction. In the combined preview it supersedes the
paragraph above's assumption that duplicate wire annotations still reach a pure Rust target.
Target-only codec symbol collisions and representation limitations remain target refusals;
their checks and the distinct-wire positive controls are retained. Reconciliation is confined
to this preview until the integration owner and the feasibility coordinator adopt it together.
