...
## Opening wave computation

The [complete original wave computation](../reviews/2026-09-06-review-boundaries-8-opening-waves.md) preserves the opening command output unchanged, including every wave, collision, unassessed entry and cycle. The output had no unassessed stories or cycles. It is a dated planning observation; two inferred paths for the unselected raw-JSON normalization story did not become its final filenames. Current published normalization tests use `crates/generate/schema-contract/tests/normalization_raw.rs` and `crates/generate/schema-contract/tests/fixtures/normalization_raw.rs`. The selected count unit and its confirmed scope are recorded above and in its governed story.
...
## First integration gate observation

At bfc2c0b804d9719bec5ee90345e9993240c37c0e, formatting and strict workspace Clippy passed. The test lane stopped with task exit 201/Cargo exit 101 after 1,549 executed cases: 1,548 passed, one failed, none ignored, across 95 summaries. The failure was the existing layout path scan reading two obsolete inferred candidate paths in the opening wave output; no writer regression was reported by that run. Later test targets and gate steps had not executed, so the partial count is not a complete green workspace result.

Root retained the raw gate output and moved the unchanged opening computation into the dated review record linked above, consistent with the existing scanner's distinction between live plans and historical citations. Its parsed JSON and fenced bytes remain identical. No path checker, normalization source, review finding or assertion was relaxed. Root will rerun the failed test lane and the remaining steps on the corrected documentation commit, preserving the successful formatting/Clippy observations on identical Rust bytes.

## Integrated result

Verification-report:review-boundaries-8-integrated records all ten gate lanes and exact source/log provenance. The complete workspace test lane at64f6f07 passed1,918cases with zero failed or ignored across156summaries. The eight remaining steps ran after the documentation-only path correction; the two earlier formatting/Clippy records retain theirbfc2c0b provenance on identical compiler inputs. Site-build actually executed the billing/WASM and lab controls, npm ci and Docusaurus build. No gate lane skipped. The full planning output is preserved verbatim in that report.
