# Review boundaries 19 — complete computed opening waves

Command: `aep plan artifact waves --kind story --status draft --format json`, actual exit 0.
This is verbatim planning evidence: scope reservations may name future files and are not claims
that those files already exist. All waves, collisions, unassessed and cycles are preserved.
