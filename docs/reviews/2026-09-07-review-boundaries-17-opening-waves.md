# Wave 17 — complete native draft selection

The following is the verbatim output of `aep plan artifact waves --kind story --status draft --format json` after the nine authored-discovery scope reservations were recorded and before its move to active. Root selects only story:review-authored-discovery. These lists include unrelated drafts because the native query is store-wide.
