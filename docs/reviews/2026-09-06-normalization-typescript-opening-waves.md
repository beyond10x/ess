# TypeScript normalization opening candidate sets

Command: `aep plan artifact waves --kind story --status draft --format json`. Observed on the published native/positional checkpoint before selecting the TypeScript story. The complete output is retained below.
...
```json
{
  ...
  "collisions": [
    ...
    {
      "a": "story:schema-unique-items-signed-zero",
      "b": "story:typescript-normalization-target",
      "path": "crates/generate/schema-contract/Cargo.toml",
      "confidence": "inferred"
    }
