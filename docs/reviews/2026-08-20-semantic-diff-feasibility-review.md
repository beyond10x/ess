...
For every consumer so far this has been an edge case. **For a diff engine it is the normal case** — holding two IRs is the entire job, and the natural code is `after.entity(&before_binding.event)`. Nothing in the type system stops it. The panic message is good, but a review tool that panics on a specification pair is reported to Operator as a crash, not as a programming mistake.
