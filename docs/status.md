...
The status report for this repository: what shipped, per wave and per component, what works today,
and what does not work yet. The front page ([`README.md`](../README.md)) carries the argument; this
page carries the bookkeeping.

Two rules keep this page honest, both learned the expensive way:

* **The gate is the measurement.** This page states no suite or test counts. In the repository's
  first 48 hours the count was hand-written into four documents — 41 suites/953 tests, 94/1693,
  106/1811, 107/1865 — and each was true when written and stale within hours. A count in prose is a
  second copy of the gate's output, and the second copy is where the drift starts. Run `task check`.
* **The tags are the record.** The delivered-waves table below is generated from the repository's
  annotated tags by `cargo xtask status`, and `status-check` in the gate fails if it drifts —
  exactly as a stale schema does. `git tag -n99` carries each wave's full message.
...
implementation conforms to its specification. See [`docs/VISION.md`](VISION.md).

Two halves is what exists. Six further designs in [`docs/design/`](design/) proposed extending
...
built as infrastructure waves, its first four delivered, with the discovery adapter outside this
...
sixth — transcript conformance, a typed specification over an agent-run transcript — was filed and
accepted on the same day, and is **in implementation as trace wave 1**.
[`docs/VISION.md`](VISION.md) § *Proposed, not accepted* is where their status is kept.
...
| `adp-domain`, `aop-domain` | 5% | 100% | the development and operations vocabularies |
...
| `ess-gen` — four projections behind one `Generator` trait | 100% | Markdown + Mermaid, JSON Schema, OpenAPI 3.1, AsyncAPI 3.0; one shared type mapping, agreement asserted |
| `ess-conformance` — the specification as an oracle | 100% | synthesis, a runner that owns its clock and id source, evidence, and thirteen deliberately wrong implementations |
| `ess-diff` — what moved, and what that invalidates | 60% | ten construct families compared field by field, with conservative canonical equality where a construct carries a predicate; impact closure with the path recoverable and invalidation that can only narrow, answering for the conformance suite and — via each artifact's `contract_digest` — for the generated artifacts. Conversions, workloads and domain naming still fall to whole-suite invalidation |
...
| `protocol infra validate\|compile\|inspect\|graph\|view\|diagnose\|simulate\|diff\|project` | 100% | an observation bundle from an external scanner becomes a validated, content-addressed IR, a typed graph, a coded diagnosis with candidates and directions, a self-contained HTML view, a three-valued simulation against a desired state, and a reviewable patch tree; nothing in this workspace reaches a cluster |
...
| [ESS wave 3.5 — reconciliation](plan/ess-wave-3.5-reconciliation.md) | 100% | all 20 gates closed, `0.3.3-ess-wave-3.5` |
...
The ESS roadmap is [`docs/plan/ess-roadmap.md`](plan/ess-roadmap.md).
...
what is unchecked is the envelope, not the types. See [`docs/plan/`](plan/) for what was built
and in what order, and [`docs/guide/`](guide/) for how to use what exists.
...
crates/
  ...
  infra-spec/       the authored desired state: twelve expectation kinds, evaluated three-valued
  infra-project/    a gap becomes a patch, an obligation or a refusal — the projection back
...
website/            the documentation site (Docusaurus); every transcript re-verified against the binary
...
Without `task`: `cargo xtask fmt --check`, `cargo xtask status --check`,
...
| Document | Role |
|---|---|
| [`docs/VISION.md`](VISION.md) | why this exists, and how its two halves compose |
| [`docs/design/consolidated-design-v0.2.md`](design/consolidated-design-v0.2.md) | **normative** — the specification for the protocol (AEP) |
| [`docs/design/reconciliation-v0.2.md`](design/reconciliation-v0.2.md) | **normative** — what is implemented, what v0.2 adds, work order, recorded deviations |
| [`docs/design/ess-implementor-design-v0.1.md`](design/ess-implementor-design-v0.1.md) | design for the Executable System Specification (ESS) — the model and the compiler are built, and a specification projects into documentation and contracts |
| [`docs/design/ess-review-v0.1.md`](design/ess-review-v0.1.md) | review of that design, with the findings that change what gets built first |
| [`docs/design/ess-closed-loop-execution-conformance-design-v0.1.md`](design/ess-closed-loop-execution-conformance-design-v0.1.md) | **implemented** as ESS wave 4 — the specification as an oracle. All four of its open decisions (D1–D4) were taken at their stated defaults |
| [`docs/design/ess-semantic-diff-impact-evolution-design-v0.1.md`](design/ess-semantic-diff-impact-evolution-design-v0.1.md) | *core implemented* as ESS wave 5 — `ess diff` and `ess impact`. The feasibility review reads the full design as four waves rather than one, and two of its seventy-eight sections were rejected outright rather than deferred |
| [`docs/design/ess-structural-synthesis-obligations-realizations-design-v0.1.md`](design/ess-structural-synthesis-obligations-realizations-design-v0.1.md) | *first slice implemented* as ESS wave 6 — the plan, the Rust emission, obligations as plan entries. The review reads the full design as four waves; §36's behavioural synthesis, §41's agent loop and §28's obligation-derived grants are rejected outright, and `Realization`, topology synthesis and formal verification stay proposed with the design |
| [`docs/design/semantic-infrastructure-discovery-specification-conformance-multicloud-design-v0.1.md`](design/semantic-infrastructure-discovery-specification-conformance-multicloud-design-v0.1.md) | *proposed, reviewed, deferred whole* — infrastructure as a fourth domain, `InfraSpec`/`InfraIr`. Two ideas harvested; the design itself would put cloud discovery adapters inside this workspace, which the vision refuses. A Kubernetes-scoped subset is planned as infrastructure waves, with the discovery adapter external to this workspace |
| [`docs/design/harness-planning-and-driver-design-v0.1.md`](design/harness-planning-and-driver-design-v0.1.md) | *Phase 1 accepted* as harness wave 1 — the markdown planning store, the `protocol artifact` verbs and the Claude Code plugin. Its Phase 2, a **reference driver** implementing the harness contract, is decided by the operator and **not accepted for build**: the vision narrowing that admits it is recorded, and the build waits behind a feasibility review |
| [`docs/design/transcript-conformance-design-v0.1.md`](design/transcript-conformance-design-v0.1.md) | *accepted, in implementation* as trace wave 1 — a typed, executable specification over an agent-run transcript: which skill was loaded, which tool was called with which arguments, what happened before what, what the run cost. The `infra-spec/1` pattern in a third observation domain, with the same `ok`/`gap`/`unk` verdicts and no model anywhere in the checker |
| [`docs/plan/trace-wave-1-transcript-checker.md`](plan/trace-wave-1-transcript-checker.md) | trace wave 1 — the transcript checker: the design's milestones T1–T3 sequenced with acceptance criteria, D1–D6 taken at their defaults, and the `regex` matcher refused by name |
| [`docs/plan/ess-wave-1-the-model.md`](plan/ess-wave-1-the-model.md) | ESS wave 1 — the model, and what its review changed |
| [`docs/plan/ess-wave-2-the-compiler.md`](plan/ess-wave-2-the-compiler.md) | ESS wave 2 — the IR, and where validation turned out to belong |
| [`docs/plan/ess-wave-3-projections.md`](plan/ess-wave-3-projections.md) | ESS wave 3 — the projections, and what they refuse to guess |
| [`docs/plan/ess-wave-3.5-reconciliation.md`](plan/ess-wave-3.5-reconciliation.md) | ESS wave 3.5 — the twenty gates wave 4 waited behind, and the evidence each one closed on |
| [`docs/plan/ess-wave-4-the-oracle.md`](plan/ess-wave-4-the-oracle.md) | ESS wave 4 — the oracle, the four model gaps it found, and the fault nothing catches (caught since wave 6.5) |
| [`docs/plan/ess-wave-5-semantic-diff.md`](plan/ess-wave-5-semantic-diff.md) | ESS wave 5 — the semantic delta and the impact closure, with what was accepted and what was rejected by name |
| [`docs/plan/ess-wave-6-structural-synthesis.md`](plan/ess-wave-6-structural-synthesis.md) | ESS wave 6 — structural synthesis: the decisions taken, and what is deliberately not in it |
| [`docs/plan/ess-wave-7-closing-the-loop.md`](plan/ess-wave-7-closing-the-loop.md) | ESS wave 7 — the loop closed over generated code, the second and third emitters, and the dual-target demonstration; W7.4 deferred |
| [`docs/plan/infra-wave-1-observe.md`](plan/infra-wave-1-observe.md) / [`infra-wave-2-analyze.md`](plan/infra-wave-2-analyze.md) / [`infra-wave-3-desired-state.md`](plan/infra-wave-3-desired-state.md) / [`infra-wave-4-project-back.md`](plan/infra-wave-4-project-back.md) | the infrastructure waves — observation and IR, then graph and diagnosis, then the desired state, then gaps projected back as patches |
| [`docs/plan/harness-wave-1-planning-plugin.md`](plan/harness-wave-1-planning-plugin.md) | harness wave 1 — the planning store, its verbs and lifecycles, the Claude Code plugin, and the driver wave it deliberately does not open |
| [`docs/plan/ess-roadmap.md`](plan/ess-roadmap.md) | the ESS waves, and what is deliberately outside them |
| [`docs/plan/wave-1-execution-core.md`](plan/wave-1-execution-core.md) | the protocol's four waves, with their acceptance criteria |
| [`docs/plan/document-authoring-brief.md`](plan/document-authoring-brief.md) | how to write a valid principle, workflow, profile or lifecycle |
| [`docs/reviews/`](reviews/) | *snapshots, not maintained* — seven independent reviews: the vision, guard efficacy, next-wave feasibility, semantic-diff feasibility, infrastructure-design feasibility and an outside pre-wave-4 readiness review, all at `3647f80`, plus a full-repository review at `95e210f`. Each describes the commit it names; where one disagrees with this page, this page is current |
...
| [`docs/design/archive/`](design/archive/) | v0.1 draft and artifact-model extension, kept for provenance |
| [`docs/guide/`](guide/) | how to adopt the protocol, wire a harness, prove a backend, and specify a system |
